...
int dude::ClampDudeX()
{
	if (x + vx + width > Graphics::ScreenWidth || x+vx<0)
		vx *= -1;
	x += vx;
	return x;
}
...
int dude::ClampDudeY()
{
	if (y + vy + height > Graphics::ScreenHeight || y+vy<0)
		vy *= -1;
	y += vy;
	return y;
}

void dude::ClampDudeVY(int max)
{
    if (vy > max)vy--;
    if(vy < -max)vy++;
...
void dude::GetInput(MainWindow& wnd) 
{
    static bool ku = false, kd = false, kr = false, kl = false;
    if (wnd.kbd.KeyIsPressed(VK_UP) && !ku) {
        vy += -1; ku = true;
    }
    else ku = false;

    if (wnd.kbd.KeyIsPressed(VK_DOWN) && !kd) {
        vy += 1; kd = true;
    }
    else kd = false;

    if (wnd.kbd.KeyIsPressed(VK_RIGHT) && !kr) {
        vx += 1; kr = true;
    }
    else kr = false;

    if (wnd.kbd.KeyIsPressed(VK_LEFT) && !kl) {
        vx += -1; kl = true;
    }
    else kl = false;
}

void dude::UpdateDude()
{
	x = ClampDudeX();
	y = ClampDudeY();
    ClampDudeVX(4);
